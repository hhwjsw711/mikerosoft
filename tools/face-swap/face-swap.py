...
--model   Optional path to inswapper_128.onnx (defaults to <repo>/../../tools/_models/face-swap/inswapper_128.onnx)
"""
...
def get_default_model_path() -> str:
    ...
    tools_dir = repo_root.parent.parent / "tools"
    return str(tools_dir / "_models" / "face-swap" / "inswapper_128.onnx")
...
def process_image(target_path, source_face, app, swapper, output_path):
    ...
    print(
        f"Found {len(target_faces)} face(s) in target image. Starting swap...",
        flush=True,
    )
    ...
    for i, face in enumerate(target_faces):
        ...
        try:
            ...
            print(
                f"WARNING: Failed to swap face {i + 1}: {e}",
                file=sys.stderr,
                flush=True,
            )
    ...
    if not success:
        print(
            f"ERROR: Could not write output: {output_path}", file=sys.stderr, flush=True
        )
        sys.exit(1)
...
def process_video(target_path, source_face, app, swapper, output_path):
    ...
    if not cap.isOpened():
        print(
            f"ERROR: Could not open video: {target_path}", file=sys.stderr, flush=True
        )
        sys.exit(1)
    ...
    while True:
        ...
        if frame_count % 10 == 0 or frame_count == 1:
            ...
            print(
                f"Processing frame {frame_count}/{total_frames} ({current_fps:.2f} fps)...",
                flush=True,
            )
            ...
            for face in target_faces:
                try:
                    result_frame = swapper.get(
                        result_frame, face, source_face, paste_back=True
                    )
                except Exception as e:
                    pass
    ...
    print(
        f"Finished processing {frame_count} frames in {total_time:.2f} seconds ({frame_count / total_time:.2f} fps).",
        flush=True,
    )
    ...
    try:
        ...
        print(
            f"ERROR: ffmpeg muxing failed: {e.stderr.decode()}",
            file=sys.stderr,
            flush=True,
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Swap a face in a target image or video"
    )
    parser.add_argument(
        "--target", required=True, help="Target image or video (face to replace)"
    )
    parser.add_argument(
        "--source", required=True, help="Source face image (donor face)"
    )
    parser.add_argument("--output", required=True, help="Output file path")
    parser.add_argument(
        "--model", default=None, help="Path to inswapper_128.onnx model"
    )
